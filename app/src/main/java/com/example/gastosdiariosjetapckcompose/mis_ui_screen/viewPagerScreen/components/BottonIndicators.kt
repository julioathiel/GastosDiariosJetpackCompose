package com.example.gastosdiariosjetapckcompose.mis_ui_screen.viewPagerScreen.components
...
fun BottonIndicators(
    ...
    val buttonText = if (pagerState.currentPage == view.size - 1) stringResource(id = R.string.ingresar) else stringResource(id = R.string.siguiente)
    Row(modifier.padding(bottom = 30.dp), horizontalArrangement = Arrangement.Center, verticalAlignment = Alignment.CenterVertically) {
        OutlinedButton(
            ...
            Text(text = stringResource(id = R.string.atras))
        }
